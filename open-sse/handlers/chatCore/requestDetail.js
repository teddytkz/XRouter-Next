...
export function saveUsageStats({ provider, model, tokens, connectionId, apiKey, endpoint, eccSkills, label = "USAGE", silent = false, latencyMs = 0 }) {
  ...
  saveRequestUsage({
    ...
    meta: eccSkills?.length ? { eccSkills } : {}
  }).catch(() => {});
}
